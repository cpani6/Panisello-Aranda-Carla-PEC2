...
# Filtrem el summarized experiment tambe 
...
Covid_75_dge <- calcNormFactors(Covid_75_dge)
Covid_75_dge$samples

# Transformem amb LogCPM
logCPM <- cpm(Covid_75_dge, log=TRUE)

# Guardem al Expression assay les dades normalitzades ()
assays(Covid_75)[["normalized"]] <- Covid_75_dge$counts

# Plot per comprovar si les dades s'han noirmalitzat be
# MA plot
par(mfrow = c(1, 2))
...
# MDS plot
plotMDS(Covid_75_dge, col = colors[group], pch = pch[group], main = "MDS plot - Perfil d'expressió")
...
plotMDS(Covid_75_dge, col=as.numeric(Covid_75$cohort))

# Veiem que outlier és 94478, el podem eliminar.

outlier_sample <- "94478"
Covid_75_dge_clean <- Covid_75_dge[, colnames(Covid_75_dge) != outlier_sample]
